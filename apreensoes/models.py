...
class Operation(models.Model):
    ...
    nome = models.CharField("nome da operação", max_length=120)
    departamento = models.CharField(max_length=120)
    responsavel = models.CharField("chefe responsável", max_length=120)
    data_operacao = models.DateField("data da operação")
    horario_previsto = models.TimeField("horário previsto", blank=True, null=True)
    local_apreensao = models.CharField("local da apreensão", max_length=255)
    ...
    suspeito_endereco = models.CharField("endereço do suspeito", max_length=255, blank=True)
...
class SeizedItem(models.Model):
    ...
    estado = models.CharField("estado / condição", max_length=80, blank=True)
    ...
    def extra_field_items(self) -> list[dict[str, str]]:
        ...
        for key, value in self.extra_data.items():
            ...
            rendered_value = "Sim" if value is True else "Não" if value is False else str(value)
            formatted.append(
                {
